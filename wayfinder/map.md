...
- [Local LLM for Smart Formatting, Backtrack, polish](issues/02-local-llm-polish.md): **llama.cpp CUDA + Qwen3-1.7B Q4_K_M primary (time-share VRAM with STT, c 512, ngl 99) + Qwen3-1.7B Q4_K_M fallback for <1s (55–75 tok/s GPU), Gemma-3-4B alt for 140-lang coverage; 600ms timeout → regex fallback; prompt preserves Taglish, never translates, handles backtrack/orthography/cursor context** — sequential STT→LLM swap fits 4GB; offline verified; prompt+samples at [research/02-local-llm-polish.md](../research/02-local-llm-polish.md). *Implementation revision 2026-08-26: c512 unworkable (SYSTEM_PROMPT alone >512 tokens → every request 400'd silently into regex); now c2048 + `/no_think`, both models resident (no swap — fits with ~740MB headroom, [eval/vram-baseline.json](../eval/vram-baseline.json)), server owned by port-health+pidfile, preloaded once at daemon startup, polish timeout 1500ms.*
...
- **Boundary corpus depth** — eval boundary gate works but corpus has 1 kamag-anak + 0 hanggang ngayon utterances; research/07 wants ≥4. Re-record via `eval/record.py` when convenient.
- **Command-mode pipeline shape** — command_release bypasses Dictation (no backtrack/context); refactor deferred until real usage shows whether it matters.
- ~~Latency proof method~~ — **resolved 2026-08-26**: `dictate()` logs per-phase JSONL to `~/.local/share/yawc/latency.log`; warm simple ~0.94s E2E, meets <1s.
- ~~Privacy proof artefact~~ — **resolved 2026-08-26**: `packaging/offline-proof.sh` PASS (pgrep widened to catch repo-run processes).
